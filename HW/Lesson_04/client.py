...
# Флаги блокировки сокета, чтоб не дёргать БД
...
class ClientSender(threading.Thread, metaclass=ClientVerifier):
    ...
    def print_history(self):
        ask = input('Показать входящие сообщения - in, исходящие - out, Enter - все ')
        with database_lock:
            if ask == 'in':
    ...
    def edit_contacts(self):
        ...
        if ans == 'del':
            ...
            # Проверка на наличие такого контакта в базе
    ...
    def run(self):
        ...
        while True:
            ...
            with sock_lock:
                ...
                # Вышел таймаут соединения если e = None, иначе обрыв соединения.
                except OSError as e:
                    if e.errno:
                        ...
                        with database_lock:
                            try:
                                self.database.save_message(message[SENDER],
                                                           self.account_name, message[MESSAGE_TEXT])
                            except Exception as e:
                                print(e)
